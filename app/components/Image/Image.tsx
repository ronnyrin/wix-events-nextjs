...
export function getImageUrlForMedia(media?: ServiceImage, width: number = media?.width ?? 640, height: number = media?.height ?? 320) {
  const imageUrl = media?.url ?
    wixMedia.getScaleToFillImageUrl(
      `wix:image://v1/${media.url}#originWidth=${width}&originHeight=${height}` + media?.url,
      width,
      height,
      {}
...
export default function Image({media, width, height}: { media?: ServiceImage; width?: number; height?: number; }) {
